...
INSERT INTO rol (id, nombre, descripcion, es_sistema) VALUES
 ...
 (2,'gerente','Gerente de sucursal: ve y opera solo su sucursal',1),
 (3,'vendedor','Atiende ventas en mostrador y pedidos online',1),
 (4,'almacenero','Gestiona stock, transferencias y recepcion de compras',1),
 (5,'repartidor','Entrega pedidos a domicilio',1),
 (6,'cliente','Compra en la tienda en linea',1);
...
INSERT INTO sucursal (id, codigo, nombre, ciudad_id, direccion, telefono, horario, es_virtual) VALUES
 (1,'SC-CENTRO','Aurora Centro',        1,'Calle Libertad 234, 1er anillo','+591 3 3334455','L-S 09:00-21:00',0),
 (2,'SC-VENTURA','Aurora Ventura Mall', 1,'Av. Banzer km 8, Ventura Mall','+591 3 3345566','L-D 10:00-22:00',0),
 (3,'LP-SOPOCACHI','Aurora Sopocachi',  4,'Av. 20 de Octubre 1850','+591 2 2445566','L-S 09:30-20:30',0),
 (4,'CB-RECOLETA','Aurora Recoleta',    6,'Av. Pando 1120','+591 4 4556677','L-S 09:00-20:00',0),
 (5,'SCZ-CD','Centro de Distribucion Nacional',1,'Parque Industrial PI-12',NULL,NULL,1);

INSERT INTO almacen (id, sucursal_id, codigo, nombre, tipo, es_principal) VALUES
 (1,1,'A-SC-CENTRO-V','Piso de venta Centro','venta',1),
 (2,1,'A-SC-CENTRO-D','Deposito Centro','deposito',0),
 (3,2,'A-SC-VENT-V','Piso de venta Ventura','venta',1),
 (4,3,'A-LP-SOPO-V','Piso de venta Sopocachi','venta',1),
 (5,3,'A-LP-SOPO-D','Deposito Sopocachi','deposito',0),
 (6,4,'A-CB-RECO-V','Piso de venta Recoleta','venta',1),
 (7,5,'A-CD-NAC','Almacen central','deposito',1),
 (8,5,'A-CD-DEV','Devoluciones nacionales','devoluciones',0);
...
INSERT INTO usuario (id, rol_id, sucursal_id, nombre, apellido, email, telefono, password_hash, email_verificado) VALUES
 (1,1,NULL,'Administrador','Aurora','admin@aurora.bo','+591 70000000',
  '$2y$12$QZBst2P7PGNT1E1H/PNxxu.h9NuQmDBdQBqwKmyW6BXC7JXVow0X6',1);
...
INSERT INTO metodo_pago (id, codigo, nombre, tipo, requiere_comprobante, disponible_offline, canal) VALUES
 (1,'efectivo','Efectivo','efectivo',0,1,'tienda'),
 (2,'qr_simple','QR Simple (BCB)','qr',1,0,'todos'),
 (3,'tarjeta_pos','Tarjeta en POS','tarjeta',0,0,'tienda'),
 (4,'tarjeta_online','Tarjeta credito/debito','tarjeta',0,0,'online'),
 (5,'transferencia','Transferencia bancaria','transferencia',1,0,'online'),
 (6,'contra_entrega','Pago contra entrega','contra_entrega',0,1,'online');
...
(
 ...
 'SELECT DATE(creado_en) AS fecha, COUNT(DISTINCT pedido_id) AS pedidos,
...
(
 ...
 'SELECT HOUR(creado_en) AS hora, COUNT(DISTINCT pedido_id) AS pedidos, SUM(subtotal) AS total
  FROM v_ventas_detalle
  WHERE creado_en BETWEEN :desde AND :hasta
    AND (:sucursal_id IS NULL OR sucursal_id = :sucursal_id)
...
(
 ...
 'SELECT CONCAT(u.nombre, " ", u.apellido) AS vendedor, s.nombre AS sucursal,
  ...
  GROUP BY u.id, vendedor, sucursal ORDER BY total DESC',
 '{"desde":{"tipo":"datetime","requerido":true},"hasta":{"tipo":"datetime","requerido":true},"sucursal_id":{"tipo":"int","requerido":false}}',
 'barras'
),
(
