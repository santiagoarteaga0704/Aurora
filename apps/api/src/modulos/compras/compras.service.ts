...
export class ComprasService {
  ...
  async crear(
    ...
    if (!proveedor?.activo) {
      throw ExcepcionNegocio.validacion({
        proveedor_id: "Ese proveedor no existe o esta de baja",
      });
    }
    ...
    if (!almacen?.activo) {
      throw ExcepcionNegocio.validacion({
        almacen_id: "Ese almacen no existe o esta inactivo",
      });
    }
    ...
    const subtotal = PreciosService.centavos(
  ...
  async recibir(
    ...
    const recibidoPor = new Map(
      (datos.items ?? []).map((i) => [i.variante_id, i.cantidad_recibida]),
    );

    for (const [varianteId, cantidad] of recibidoPor) {
      const linea = compra.compra_detalle.find(
        (d) => d.variante_id === varianteId,
      );
      if (!linea) {
        throw ExcepcionNegocio.validacion({
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const linea of compra.compra_detalle) {
        ...
        const subtotalLinea = PreciosService.centavos(
          cantidad * aNumero(linea.costo_unitario),
        );
  ...
  async listar(filtros: DatosConsultaCompras, usuario: UsuarioAutenticado) {
    ...
    return {
      total,
      items: filas.map((c): CompraResumen => ({
        id: c.id.toString(),
        numero: c.numero,
        estado: c.estado,
  ...
  private async verificarVariantes(ids: number[]): Promise<void> {
    const existen = await this.prisma.variante.count({
      where: { id: { in: ids } },
    });
    if (existen !== new Set(ids).size) {
      throw ExcepcionNegocio.validacion({
        items: "Alguna de las variantes no existe",
      });
    }
  }
