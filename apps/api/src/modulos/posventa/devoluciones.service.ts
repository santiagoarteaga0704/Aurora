...
} from "../../nucleo/bitacora/bitacora.service";
...
export class DevolucionesService {
  ...
  async solicitar(
    ...
    const esDueno =
      pedido.cliente_id !== null && pedido.cliente_id === usuario.id;
    const esPersonal = this.permisos.puede(
      usuario.permisos,
      PERMISOS.DEVOLUCION_GESTIONAR,
    );
    ...
    const porLinea = new Map(
      pedido.pedido_detalle.map((d) => [d.id.toString(), d]),
    );
    ...
    for (const item of datos.items) {
      ...
      const disponible =
        linea.cantidad - (yaDevuelto.get(item.pedido_detalle_id) ?? 0);
  ...
  async resolver(
    ...
    if (devolucion.cliente_id !== null) {
      await this.notificaciones.crear({
        ...
        tipo: "devolucion",
        titulo: datos.aprobada
          ? "Aprobamos tu devolucion"
          : "No pudimos aprobar tu devolucion",
  ...
  async recibir(
    ...
    const almacenDevoluciones = await this.almacenDeDevoluciones(
      devolucion.pedido.sucursal_id,
    );
    ...
    await this.prisma.$transaction(async (tx) => {
      for (const linea of devolucion.devolucion_detalle) {
        ...
        await tx.devolucion_detalle.update({
          where: { id: linea.id },
          data: {
            estado_prenda: clasif.estado_prenda,
        ...
        reingresadas.push(
          `${linea.cantidad} x variante ${linea.variante_id} (${clasif.estado_prenda})`,
        );
      }
  ...
  async listar(
    filtros: DatosConsultaDevoluciones,
    usuario: UsuarioAutenticado,
  ) {
    const esPersonal = this.permisos.puede(
      usuario.permisos,
    ...
    return {
      total,
      items: filas.map((d): DevolucionResumen => ({
        id: d.id.toString(),
        numero: d.numero,
        pedido_numero: d.pedido.numero,
        cliente: d.cliente
          ? `${d.cliente.usuario.nombre} ${d.cliente.usuario.apellido}`
          : null,
        motivo: d.motivo,
        estado: d.estado,
    ...
    if (esPersonal) {
      ...
      if (propia !== null && propia !== d.pedido.sucursal_id) {
        throw ExcepcionNegocio.sinPermiso(
          "Esa devolucion no es de tu sucursal",
        );
      }
    }
  ...
  private async buscar(id: bigint, usuario: UsuarioAutenticado) {
    const d = await this.prisma.devolucion.findUnique({
      ...
      include: {
        devolucion_detalle: {
          include: { pedido_detalle: { select: { precio_unitario: true } } },
        },
        pedido: {
          select: { sucursal_id: true, almacen_id: true, numero: true },
        },
      },
    });
    if (!d) throw ExcepcionNegocio.noEncontrado("Esa devolucion no existe");
  ...
  private async montoDevuelto(
    lineas: readonly {
      cantidad: number;
      pedido_detalle: { precio_unitario: unknown };
    }[],
  ): Promise<number> {
    return PreciosService.centavos(
      lineas.reduce(
        (s, l) =>
          s + l.cantidad * aNumero(l.pedido_detalle.precio_unitario as never),
        0,
      ),
    );
  }
  ...
  private async almacenDeDevoluciones(
    sucursalId: number,
  ): Promise<number | null> {
  ...
  private async seDevolvioTodo(
    ...
    const devuelto = await tx.devolucion_detalle.aggregate({
      where: {
        devolucion: {
          pedido_id: pedidoId,
          estado: { in: ["recibida", "reembolsada"] },
        },
      },
      _sum: { cantidad: true },
    });
